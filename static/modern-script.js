...
function enhanceGameWithSoundEffects() {
    ...
    if (!document.getElementById('true-button')) {
        return;
    }
    ...
    // Store the original updateScore function
    const originalUpdateScore = window.updateScore;

    // Override the updateScore function to show donation prompt at certain scores
    window.updateScore = function() {
        ...
        // At certain milestone scores, highlight the donation button
        if (currentScore === 5 || currentScore === 10 || currentScore === 15) {
            highlightDonation(currentScore);
        }
    };
...
// Function to highlight the donation button at milestone scores
function highlightDonation(score) {
    const supportDev = document.querySelector('.support-developer');
    if (!supportDev) {
        return;
    }

    // Create a special message based on the score
    let message = '';
    if (score === 5) {
        message = 'Great start! Support the developer to unlock more features!';
    } else if (score === 10) {
        message = 'You\'re on fire! Buy me a coffee to keep the game running!';
    } else if (score === 15) {
        message = 'Amazing score! Consider supporting this project!';
    }

    // Update the message
    const messagePara = supportDev.querySelector('p');
    if (messagePara) {
        messagePara.textContent = message;
    }

    // Make it more noticeable
    supportDev.style.animation = 'none';
    setTimeout(() => {
        supportDev.style.animation = 'pulse 0.7s infinite';
        supportDev.style.backgroundColor = 'rgba(255, 215, 0, 0.3)';

        // If on mobile, scroll to it
        if (window.innerWidth < 768) {
            supportDev.scrollIntoView({ behavior: 'smooth', block: 'end' });
        }

        // Reset after 5 seconds
        setTimeout(() => {
            supportDev.style.animation = 'pulse 2s infinite';
            supportDev.style.backgroundColor = 'rgba(255, 255, 255, 0.15)';
        }, 5000);
    }, 10);
}
...
function setupConfettiEffect() {
    ...
    // Override the endGame function to add confetti for winners and show donation prompt
    window.endGame = function(message) {
        if (message.includes('WIN')) {
            createConfetti();
        }

        // Mark that the game has been played to show donation prompt
        document.body.classList.add('game-played');

        // Make the support developer section more noticeable
        setTimeout(() => {
            const supportDev = document.querySelector('.support-developer');
            if (supportDev) {
                supportDev.style.animation = 'pulse 1s infinite';
